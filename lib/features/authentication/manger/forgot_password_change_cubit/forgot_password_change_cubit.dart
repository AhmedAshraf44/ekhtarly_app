...
class ForgotPasswordChangeCubit extends Cubit<ForgotPasswordChangeState> {
  ForgotPasswordChangeCubit(this.authRepo)
      : super(ForgotPasswordChangeInitial());
  ...
  Future<void> forgotPasswordChange(
    ...
    var result = await authRepo.forgotPasswordChange(
      email: email,
      code: code,
      password: password,
    );
    ...
    result.fold(
      (failure) =>
          emit(ForgotPasswordChangeFailure(errorMessage: failure.errorMessage)),
      (resned) => emit(
